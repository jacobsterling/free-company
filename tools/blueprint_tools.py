...
def send_command(command_name, params=None):
    """Send a command to the MCP server."""
    if params is None:
        params = {}

    command = {"command": command_name, "params": params}
...
def create_blueprint(name, parent_class):
    """Create a new blueprint with the given name and parent class."""
    return send_command("CreateBlueprint", {"name": name, "parentClass": parent_class})
...
def add_component(
    ...
    return send_command("AddComponent", params)


def add_input_mapping(name, type, scale, keys):
    """Add an input mapping to the project settings."""
    return send_command(
        "AddInputMapping", {"name": name, "type": type, "scale": scale, "keys": keys}
    )
